...
`C:\Users\jzdxjk\Documents\字幕云翻译\HANDOFF.md` — 暗色 NAS 风 UI + Mac Dock 式 4 视图导航 + 主界面 Emby 风格横版海报墙（按日期分组 + 横向滚动行）+ 海报后端代理 + localStorage 持久化缓存 + 增量渲染 + 亮/暗主题 + PWA 移动端底部 Dock 重做 + Docker Hub 发布 + Cloudflare 探索（已回滚）+ 取消杀进程 + 失败不重试 + 保存 toast + 版本号 + 番号规范化搜索回退 + 根目录直放文件迁移修复 + FC2番号格式映射；win 预览端口 15678；v2.4 修完 FC2 番号提取和海报搜索问题；下一步 → LLM 翻译模块。
...
### v2.4 — FC2番号格式映射修复（2026-05-28）

- **bug**：FC2番号有三种格式（`FC2PPV-4907804`、`FC2-PPV-4907804`、`FC2-4907804`），DBO只认 `FC2-4907804` 格式，导致海报搜索失败
- **根因**：
  1. `AV_PATTERN` 正则表达式对 `FC2PPV-4907804` 匹配错误，提取出 `2PPV-49078`
  2. `normalize_av_code()` 和 `_normalizeAvCode()` 函数对FC2只返回小写，没有做格式映射
- **修复（media.py）**：
  - 修改 `AV_PATTERN` 正则表达式，正确匹配三种FC2格式
  - 修改 `normalize_av_code()` 函数，FC2番号统一映射为 `fc2-数字` 格式
- **修复（app.js）**：
  - 修改 `_normalizeAvCode()` 函数，FC2番号统一映射为 `fc2-数字` 格式
  - 清除localStorage中FC2番号的旧缓存（null值）
  - 优先从output_files提取av码，解决子文件夹媒体文件海报问题
- **映射规则**：
  - `FC2PPV-4907804` → `fc2-4907804`（搜索时）
  - `FC2-PPV-4907804` → `fc2-4907804`（搜索时）
  - `FC2-4907804` → `fc2-4907804`（搜索时）
- **重要**：番号显示保持原样，只在搜索海报时做格式映射
- **Docker Hub**：`jzdxjk/subtitle-modal-web:v2.4`
...
> **2026-05-28**：v2.4 FC2番号格式映射修复完成，下一步继续LLM翻译模块。
...
| 功能 | 状态 |
|---|---|
| FC2 非标番号适配 | ✅ v1.9 完成 + v2.4 修复格式映射 |
| 任务删除 | ✅ v1.9 完成 |
| 海报加载骨架屏 | ✅ v1.9 完成 |
